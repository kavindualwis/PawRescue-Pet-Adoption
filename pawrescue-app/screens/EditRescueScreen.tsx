...
export const EditRescueScreen = () => {
  ...
  const loadRescueCase = async (caseId: string) => {
    try {
      ...
      Alert.alert('Error', 'Could not load the rescue case details');
      router.back();
    } finally {
      setInitialLoad(false);
    }
  ...
  useEffect(() => {
    if (!initialLoad && placesRef.current && location) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      await rescueService.updateRescueCase(id as string, {
      ...
      console.log("Emitting 'rescueDataChanged' from Edit Screen...");
      DeviceEventEmitter.emit('rescueDataChanged');
      
      Alert.alert('Success', 'Rescue case updated successfully.', [
        { text: 'OK', onPress: () => router.back() }
      ]);
    } catch (error: any) {
      Alert.alert('Error', error.message || 'Failed to update rescue case');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <View style={[styles.header, { borderBottomColor: colors.text + '10' }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Edit Rescue Case</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
        <Text style={[styles.infoText, { color: colors.text + '80' }]}>Update the details of the rescue case.</Text>
        ...
        >
          {loading ? (
            ...
            <Text style={styles.submitBtnText}>Update Rescue Case</Text>
          )}
        </TouchableOpacity>
          </>
        }
